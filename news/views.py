...
class Category_details(ListView):
    ...
    def get_queryset(self):
        ...
        posts = Post.objects.filter(category=category, active=True).order_by('-publish')
        return posts
    
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["category_detail"] = get_object_or_404(Category, slug=self.kwargs['slug'])
        print(context["category_detail"])
        return context
...
def create_post(request):
    ...
    if request.method == 'POST':
        ...
        form_post = PostForm()
    return render(request, 'news/mange/form.html', {'form_post': form_post })  
